    ...
    if current_step == 0:
        ...
        if structured_llm is None:
            ...
            try:
                ...
                missing = [f for f in ["name", "email", "phone"] if not lead_data.get(f)]
                if missing:
                ...
                for change in entry.get('changes', []):
                    ...
                    if value.get('messages'):
                        for msg_obj in value.get('messages', []):
                            ...
                            if from_num and msg_type == 'text':
                                msg_body = msg_obj['text']['body']
                                print(f'[WEBHOOK_POST] Processing text from {from_num}: "{msg_body}"')
                                response_text = process_message(msg_body, from_num)
                                print(f'[WEBHOOK_POST] Generated response for {from_num}: "{response_text}"')
                                if response_text:
                                    send_whatsapp_message(from_num, response_text)
                                else:
                                    print(f"[WEBHOOK_POST] No response for {from_num}.")
                            elif from_num:
                                print(f"[WEBHOOK_POST] Non-text type '{msg_type}' from {from_num}.")
                            # else: print("[WEBHOOK_POST] Msg w/o 'from_num' or not text.") # Too verbose
